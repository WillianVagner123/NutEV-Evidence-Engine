...
def test_retry_at_higher_dpi_recovers(monkeypatch, tmp_path):
    # A "scanned" PDF: first DPI yields garbage, higher DPI yields good text.
    p = tmp_path / "scan.pdf"
...
def test_no_retry_when_first_pass_is_good(monkeypatch, tmp_path):
    ...
    assert calls["dpi"] == [300]        # first pass sufficient, no retry


def test_ocr_scanned_pdf_pages_returns_per_page(monkeypatch, tmp_path):
    p = tmp_path / "scan.pdf"
    p.write_bytes(b"%PDF-1.4 fake")
    monkeypatch.setattr(pdf_text, "is_probably_pdf_file", lambda _p: True)
    monkeypatch.setattr(
        pdf_text, "_ocr_pages_at_dpi",
        lambda path, dpi, logger: (["Page one about diet quality.", "Page two on adherence."], []),
    )
    pages, failed = pdf_text.ocr_scanned_pdf_pages(p, _LOG)
