...
def test_pdf_keeps_pdf_status(tmp_path, monkeypatch):
    class R: status_code=200; headers={'Content-Type':'application/pdf'}; content=b'%PDF-1.7 abc'; text=''; url='https://x/a.pdf'
    monkeypatch.setattr('requests.get', lambda *a, **k: R())
    s=_s(tmp_path)
    rows,_=capture_watch_items([{"document_id":"d1","url":"https://x/a.pdf","title":"x"}],s,setup_logger(s.output_dirs['07_logs']),'run1','quick')
    assert rows[0]['download_status']=='pdf'


def test_html_keeps_html_snapshot_status(tmp_path, monkeypatch):
    class R: status_code=200; headers={'Content-Type':'text/html'}; content=b'<html><title>T</title><h1>H</h1><body>Body</body></html>'; text='<html><title>T</title><h1>H</h1><body>Body</body></html>'; url='https://x/a'
    monkeypatch.setattr('requests.get', lambda *a, **k: R())
    s=_s(tmp_path)
    rows,_=capture_watch_items([{"document_id":"d1","url":"https://x/a","title":"x"}],s,setup_logger(s.output_dirs['07_logs']),'run1','quick')
    assert rows[0]['download_status']=='html_snapshot'


def test_http_404_metadata_only(tmp_path, monkeypatch):
    class R: status_code=404; headers={'Content-Type':'text/html'}; content=b'x'; text='x'; url='https://x/a'
    monkeypatch.setattr('requests.get', lambda *a, **k: R())
    s=_s(tmp_path)
    rows,_=capture_watch_items([{"document_id":"d1","url":"https://x/a","title":"x"}],s,setup_logger(s.output_dirs['07_logs']),'run1','quick')
