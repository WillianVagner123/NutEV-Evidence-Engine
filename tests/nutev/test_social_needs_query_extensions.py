...
def test_provider_queries_include_food_farmacy_variants_for_busca2b():
    tax = {
        "global": {
            "document_types": {"reviews": ["systematic review"]},
            "implementation_behavior": {"behavioral": ["behavior change"]},
    ...
    assert any("food farmacy" in query or "fresh food farmacy" in query for query in queries)
    assert any(
        "food farmacy program" in query
        or "food farmacy intervention" in query
        or "fresh food farmacy program" in query
        or "fresh food farmacy intervention" in query
        for query in queries
    )
