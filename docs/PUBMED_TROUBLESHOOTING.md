...
Se um checkpoint JSON estiver corrompido, NutEV ignora o arquivo, tenta renomeá-lo com sufixo `.corrupt` e reinicia aquela query com segurança.

## Abstract ausente

`esummary.fcgi` nem sempre contém abstract. Defina `NUTEV_PUBMED_FETCH_ABSTRACTS=1` para ativar `efetch.fcgi` por lote e preencher abstracts completos quando disponíveis. Se o XML do efetch vier inválido, a busca continua com metadados de esummary.
