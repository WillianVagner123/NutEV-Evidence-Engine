...
| 2 | Search too broad (pharmacology, oncology VTE, pediatrics, off-scope) | **Config/tuning** | Needs busca1 query tightening + exclusion terms + a diet-centrality gate. Scoped as next PR (see below). |
| 3 | Only 12 official sources | **Data** | Official search must be built by **country / region / institution**, not as a complement. Needs a curated `official_sources_manifest` expansion (requires your input on target countries/bodies). |
...
| 7 | Version/duplication (GLP-1 parallel pubs; annual Standards of Care) | **Config/tuning** | Dedup must distinguish duplicate / parallel publication / guideline chapter / parent doc / annual version. Scoped as next PR. |
| 8 | Priority texts blocked (403: Diabetes Journals, OUP, Wiley, MDPI, LWW, NEJM) | **Partly by design** | The tool deliberately does not bypass paywalls. Next step: resolve legal **open-access copies** (Unpaywall / OpenAlex OA location / Europe PMC full text / PMC) before giving up. Scoped as next PR. |
