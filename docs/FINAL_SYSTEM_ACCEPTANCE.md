...
## Last observed regression

The next browser validation caught a null-DOM callback error during logout after its eleven functional assertions had passed. The production client was corrected to hide/inert the retiring document without removing pending-handler nodes before navigation commits; stale responses still fail closed. A deterministic lifecycle regression was added and the full local 1,045-test suite passed. Final-head Chromium, including zero unexpected page errors, remains mandatory; see the later verified PR comment.
