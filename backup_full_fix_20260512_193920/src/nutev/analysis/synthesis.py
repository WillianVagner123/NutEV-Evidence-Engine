...
def build_master_rows(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        ...
        master.append({
            ...
            "year": r.get("year", "") or infer_year((r.get("title","") + " " + text)),
            "score": r.get("relevance_score", 0),
            "doc_type": r.get("doc_type", "unknown") if r.get("doc_type") else infer_doc_type(r.get("url",""), r.get("title","")),
        ...
        for key, typ in map_types:
            if key in low:
                rows.append({
                    ...
                    "prioridade": "alta" if r.get("score",0) >= 8 else "media",
                    "tipo": typ,
...
def write_synthesis_outputs(master_rows: list[dict], out_dir: Path) -> None:
    ...
    with pd.ExcelWriter(out_dir / "NUTEV_EVIDENCE_MASTER.xlsx") as w:
        for ws in ["busca1", "busca2a", "busca2b", "a3"]:
            write_excel_sheet(w, df[df["workstream"] == ws], f"{ws}_documents")
        write_excel_sheet(w, df.sort_values("score", ascending=False).head(200), "top_ranked")
        write_excel_sheet(w, df.groupby("source", dropna=False).size().reset_index(name="n"), "by_source")
        write_excel_sheet(w, df.groupby("year", dropna=False).size().reset_index(name="n"), "by_year")
        by_domain = df.assign(domain=df["domains"].fillna("").str.split(";")).explode("domain").groupby("domain", dropna=False).size().reset_index(name="n")
        by_diet = df.assign(diet=df["diet_pattern"].fillna("").str.split(";")).explode("diet").groupby("diet", dropna=False).size().reset_index(name="n")
        by_cond = df.assign(cond=df["clinical_condition"].fillna("").str.split(";")).explode("cond").groupby("cond", dropna=False).size().reset_index(name="n")
        write_excel_sheet(w, by_domain, "by_domain")
        write_excel_sheet(w, by_diet, "by_diet_pattern")
        write_excel_sheet(w, by_cond, "by_clinical_condition")
    ...
    write_excel_file(df.groupby("source", dropna=False).size().reset_index(name="n"), out_dir / "NUTEV_SOURCE_SUMMARY.xlsx")
    domain_df = df.assign(domain=df["domains"].fillna("").str.split(";")).explode("domain").groupby("domain", dropna=False).size().reset_index(name="n")
    write_excel_file(domain_df, out_dir / "NUTEV_DOMAIN_SUMMARY.xlsx")