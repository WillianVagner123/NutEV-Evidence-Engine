...
def save_capture_json(item: dict, out_dir: Path) -> dict:
    ...
    payload = {
        ...
        "content_type": item.get("content_type"), "http_status": item.get("http_status"), "abstract": item.get("abstract"),
        "headings": item.get("headings", []), "doi_candidates": item.get("doi_candidates", []), "pdf_links_found": item.get("pdf_links_found", []),
    }
...
def capture_single_watch_item(item: dict, settings, logger, run_id: str) -> dict:
    ...
    try:
        ...
        txt_path.write_text(extracted["clean_text"], encoding="utf-8", errors="ignore")
...
def capture_watch_items(items: list[dict], settings, logger, run_id: str, mode: str, capture_limit: int | None = None) -> tuple[list[dict], list[dict]]:
    logs = settings.output_dirs["07_logs"]
