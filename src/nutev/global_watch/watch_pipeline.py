...
def _provider_query(base: str, provider: str, since_days: int) -> str:
    date_from = (datetime.now(timezone.utc).date() - timedelta(days=max(since_days, 0))).isoformat()
    if provider == "pubmed":
        return f"({base}) AND (\"{date_from}\"[Date - Publication] : \"3000\"[Date - Publication])"
...
def infer_workstream_affinity(title: str, category: str) -> list[str]:
    text = f"{title} {category}".lower(); out=[]
    if any(k in text for k in ["guideline", "diet", "food", "nutrition"]): out.append("busca1")
    if any(k in text for k in ["obesity", "cardio", "metabolic", "diabetes"]): out.append("busca2a")
    if any(k in text for k in ["adherence", "implementation", "behavior", "framework", "instrument"]): out.append("busca2b")
    if any(k in text for k in ["framework", "questionnaire", "competencies"]): out.append("a3")
    return out or ["busca1"]


def normalize_watch_hit(raw_hit: dict, provider_name: str, category: str, query: str, since_days: int = 30) -> dict:
    title = raw_hit.get("title") or ""
    year = raw_hit.get("year")
    try: year = int(year) if year is not None else None
    except Exception: year = None
    is_recent = bool(year and year >= datetime.now(timezone.utc).year - (1 if since_days <= 365 else 2))
    item = {
        "title": title, "url": raw_hit.get("url") or "", "doi": raw_hit.get("doi") or "", "year": year,
        "source_provider": provider_name, "category": category, "query": query,
        "evidence_type": infer_evidence_type(title, raw_hit.get("abstract") or "", raw_hit.get("url") or ""),
        "workstream_affinity": infer_workstream_affinity(title, category),
        "download_status": "metadata_only", "relevance_score": 50, "failure_reason": "", "fallback_used": False,
...
def run_watch_provider(provider_name: str, query: str, category: str, logger, run_id: str, logs_dir: Path, since_days: int) -> list[dict]:
    ...
    try:
        ...
        out = [normalize_watch_hit(r, provider_name, category, query, since_days) for r in rows]
...
def run_global_watch(settings, logger, since_days, mode, resume, official_crawl, country_discovery, llm_enabled, capture_enabled=False, capture_limit=None, notify_webhook=False, webhook_url=None):
    ...
    queries = build_watch_queries([], since_days, mode); providers = ["pubmed", "europepmc", "openalex", "crossref"]; rows=[]
    for q in queries[:8 if mode == "quick" else 20 if mode == "thesis" else 30]:
        for provider in providers:
            rows.extend(run_watch_provider(provider, q["query"], q["category"], logger, run_id, logs, since_days))
        ...
        try:
            ...
            for ws in ["busca1", "busca2a", "busca2b", "a3"]:
                for r in manifest_sources(manifest, ws):
                    rows.append(normalize_watch_hit(r, "official_sources", "guidelines_consensus", "official manifest", since_days))
        except Exception as exc:
    ...
    if not rows:
        fallback = normalize_watch_hit({"title": "global watch fallback item", "url": "https://fallback.local", "doi": ""}, "watch_seed", "guidelines_consensus", "fallback", since_days)
        fallback["fallback_used"] = True; fallback["failure_reason"] = "no_provider_results"; rows = [fallback]

    rows = list({r["document_id"]: r for r in rows}.values())
    seen_path = settings.project_root / "09_global_watch" / "watch_state" / "seen_items.json"; seen = load_seen_items(seen_path) if resume else {}
    rows = mark_new_items(rows, seen)
    for r in rows:
        r["is_new_to_system"] = bool(r.get("is_new"))
    ...
    run_dir = settings.project_root / "09_global_watch" / "runs" / datetime.now(timezone.utc).date().isoformat(); new_rows = [r for r in rows if r.get("is_new")]
    ...
    ids={r.get("document_id") for r in rows[:10]}
    for r in rows: r["webhook_included"]=r.get("document_id") in ids and wh.get("status")=="sent"
    ...
    job.status="completed"; job.finished_at=datetime.now(timezone.utc)
