...
def run_dashboard(project_root: Path) -> None:
    ...
    page = st.sidebar.radio('Pages', ['Overview','Pipeline Status','Global Evidence Matrix','Audit Engine','Recommendation Candidates','Human Review Queue','Logs & Run Summary','Methods Preview','Provider Settings'])
    ...
    if page == 'Overview':
        ...
        st.warning('As chaves são usadas apenas localmente. O NutEV não aprova recomendações finais com LLM.')
        reg = load_provider_registry(Path('config')).get('providers', [])
        settings = load_provider_settings(project_root).get('providers', {})
        llm_enabled = sum(1 for p in reg if p.get('provider_type') == 'llm' and settings.get(p.get('provider_id'), {}).get('enabled'))
        search_cfg = sum(1 for p in reg if p.get('provider_type') == 'search' and settings.get(p.get('provider_id'), {}).get('enabled'))
        bib_cfg = sum(1 for p in reg if p.get('provider_type') == 'bibliographic' and settings.get(p.get('provider_id'), {}).get('enabled'))
        st.write({'llm_enabled_count': llm_enabled, 'search_configured': search_cfg, 'bibliographic_configured': bib_cfg})
        for p in reg:
            pid = p['provider_id']
            cfg = settings.get(pid, {})
            st.subheader(p.get('label', pid))
            st.write({'type': p.get('provider_type'), 'enabled': cfg.get('enabled', False), 'mode': cfg.get('mode', 'disabled'), 'model': cfg.get('model', ''), 'secret_status': 'configured' if cfg.get('api_key') or cfg.get('env_var') else 'missing'})
