    ...
    if original_build is not None and not getattr(original_build, "_nutev_foodmed_patched", False):
        def wrapped_build(keyword_taxonomy: dict, workstream: str) -> list[str]:
            ...
            if ws == "busca2b":
                ...
                queries.append('("type 2 diabetes remission" OR "diabetes remission" OR "weight loss maintenance" OR "weight regain prevention" OR "dietary self-monitoring") AND ("nutrition" OR "dietary" OR "obesity" OR "cardiometabolic")')
            elif ws in {"busca1", "artigo3_framework"}:
