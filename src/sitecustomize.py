    ...
    if original_render is not None and not getattr(original_render, "_nutev_terms_patched", False):
        def wrapped_render(keyword_taxonomy: dict, workstream: str, provider: str) -> list[str]:
            ...
            if provider == "pubmed":
                ...
                if ws == "busca2a":
                    terms = ["therapeutic lifestyle changes", "mediterranean dietary pattern", "living guideline", "clinical guidance"]
                elif ws == "busca2b":
                    terms = ["medical nutrition therapy", "hybrid type 1", "hybrid type 2", "hybrid type 3", "steatotic liver disease", "metabolic dysfunction-associated steatohepatitis", "ketogenic diet", "low-carbohydrate diet", "network meta-analysis", "living systematic review"]
                else:
                    terms = []
