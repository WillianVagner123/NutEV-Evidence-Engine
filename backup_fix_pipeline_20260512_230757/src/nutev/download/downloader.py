...
SCHOLARLY_HTML_HOSTS = {
    "springer.com", "nature.com", "cambridge.org", "frontiersin.org", "karger.com",
    "oup.com", "ahajournals.org", "pubmed.ncbi.nlm.nih.gov", "pmc.ncbi.nlm.nih.gov",
    "scielo.br", "cureus.com", "dovepress.com", "diabetesjournals.org",
    "lifestylemedicine.org", "who.int", "heart.org", "abccardiol.org", "bmj.com"
}
...
def _looks_html_candidate(url: str, source: str | None = None) -> bool:
    ...
    if _host_matches(url, SCHOLARLY_HTML_HOSTS):
        return True
    htmlish = [
        "/article", "/articles", "/doi", "/content", "/journal", "/journals",
        "guideline", "statement", "consensus", "framework", "questionnaire", "recommendation"
    ]
...
def _save_snapshot_html(
    ...
    try:
        ...
        r = session.get(candidate_url, timeout=30, allow_redirects=True)
        r.raise_for_status()
    except Exception as e:
    ...
    for rec in records:
        ...
        if should_download(resolved_url, ext, rec.get("source")):
            ...
            try:
                ...
                snap = _save_snapshot_html(s, resolved_url, rec, public_dir, official_dir, dedup, logger)
                if snap:
                    man.append(snap)
                    continue

                if raw_url != resolved_url:
                    snap = _save_snapshot_html(s, raw_url, rec, public_dir, official_dir, dedup, logger)
                    if snap:
                        man.append(snap)
                        continue
