...
def run_pipeline(settings: NutevSettings, workstreams: list[str], logger) -> dict[str, int]:
    ...
    for ws, queries in qpack.items():
        rows=[]
        for q in queries[:3]:
            rows += search_openalex(q) + search_europepmc(q) + search_pubmed(q)
        rows += manifest_sources(sources, ws)
        for r in rows: r["workstream"] = ws
        rows = [score_record(r, scoring, ws) for r in rows]
        manifest, failed = download_records(rows, settings.output_dirs["03B_public_downloads"], settings.output_dirs["03C_official_docs"], logger)
        all_manifest += manifest
        total_downloads += len(manifest); total_failed += len(failed)
        ...
        ext_by_url = {}
        for m in manifest:
            e = extract_document(Path(m["path"]), settings.output_dirs["04_ocr_text"], settings.output_dirs["05_extraction"], logger)
            extraction_manifest.append(e); total_ocr += int(e.get("used_ocr", False)); ext_by_url[m["url"]] = e
        for r in rows:
            url_key = r.get("url")
            if not isinstance(url_key, str):
                url_key = ""
            e = ext_by_url.get(url_key, {})
            r["file_path"] = e.get("file", ""); r["ocr_status"] = "used" if e.get("used_ocr") else "not_used"; r["extraction_status"] = e.get("extraction_status", "missing")
            if e.get("text_path"): r["extracted_text"] = Path(e["text_path"]).read_text(encoding="utf-8", errors="ignore")
        if ws == "busca1": rows = domains_busca1.apply_domain_rules(rows, load_json(settings.config_root / "domain_rules_busca1.json"))
        elif ws == "busca2a": rows = domains_busca2a.apply_domain_rules(rows, load_json(settings.config_root / "domain_rules_busca2a.json"))
        elif ws == "busca2b": rows = domains_busca2b.apply_domain_rules(rows, load_json(settings.config_root / "domain_rules_busca2b.json"))
        elif ws == "a3": rows = build_framework_signals(rows)
    ...
    summary = {"workstreams": workstreams, "records": len(all_rows), "downloads_ok": total_downloads, "downloads_failed": total_failed, "ocr_docs": total_ocr}
