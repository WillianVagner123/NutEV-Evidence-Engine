...
## Keyword Taxonomy v4 + Smart Query Builder

O Global Watch agora usa `config/keyword_taxonomy.json` (versão `2026-05-v4`) com vocabulário controlado, termos de update, instituições/sociedades internacionais, termos multilíngues e instrumentos/questionários.

As queries são geradas por intenção (`guideline`, `review`, `trial`, `official`, `instrument`, `update`, `country`) com diferenças por modo (`quick`, `thesis`, `exhaustive`) e exclusões de ruído (`negative_noise_terms`).
