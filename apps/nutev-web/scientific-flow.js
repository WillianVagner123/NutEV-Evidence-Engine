...
const FLOW_STAGES=[
  {id:'map',label:()=>t('Mapa de Evidências','Evidence Map'),note:()=>t('Estrutura do corpus','Corpus structure'),href:'/evidence-map.html'},
  {id:'intelligence',label:()=>t('Análise de Evidências','Evidence Analysis'),note:()=>t('Inspeção de sinais','Signal inspection'),href:'/intelligence.html'},
  {id:'review',label:()=>t('Revisão Humana','Human Review'),note:()=>t('Decisão explícita','Explicit decision'),href:'/review.html'}
]
...
function installFlowRail(){
  if(!flowStage)return
  document.querySelector('#scientificFlowRail')?.remove()
  ...
  section.innerHTML=`
    <div class="scientific-flow-head">
      <div><span class="scientific-flow-eyebrow">${t('FLUXO CIENTÍFICO','SCIENTIFIC WORKFLOW')}</span><strong>${t('Do mapa à decisão humana, sem atalhos científicos','From map to human decision, without scientific shortcuts')}</strong></div>
      <span class="scientific-flow-badge">${t('somente navegação','navigation only')}</span>
    </div>
    <div class="scientific-flow-stages" role="navigation" aria-label="${t('Fluxo científico visual','Visual scientific workflow')}">
    ...
    <div class="scientific-flow-guardrail">${t('Mapa e Análise organizam navegação e inspeção. Nenhum filtro, volume, recorrência ou clique cria elegibilidade, inclusão, certeza, EvidenceClaim, decisão de revisão ou PRISMA.','Map and Analysis organize navigation and inspection. No filter, volume, recurrence or click creates eligibility, inclusion, certainty, an EvidenceClaim, a review decision or PRISMA.')}</div>`
...
function updateMapContext(){
  ...
  const summary=$flow('#mapFilterSummary')?.textContent?.replace(/\s+/g,' ').trim()||t('Tier A estrutural','structural Tier A')
  const domain=$flow('#mapDomainFilter')?.value||''
  const context=$flow('#scientificFlowContext')
  if(context){
    context.innerHTML=`<div><strong>${t('Recorte atual','Current slice')}</strong><span>${flowEsc(summary)}</span></div><a class="scientific-flow-action" href="${domain?`/intelligence.html?domain=${encodeURIComponent(domain)}`:'/intelligence.html'}">${t('Levar domínio para Análise →','Carry domain to Analysis →')}</a>`
  }
  refreshFlowLinks()
}
...
function renderIntelligenceOverview(){
  ...
  overview.innerHTML=`<div class="scientific-domain-overview-head"><div><strong>${t('Panorama do domínio','Domain overview')}</strong><span>${t('Volume estrutural + cobertura de pacotes de resultados; não é força da evidência.','Structural volume + result-bundle coverage; it is not evidence strength.')}</span></div><span>${rows.length} ${t('domínios','domains')}</span></div><div class="scientific-domain-bars">${rows.map(item=>{
    const volume=Math.max(3,100*item.documents/max)
    const ready=item.documents?100*item.ready/item.documents:0
    return `<button type="button" class="scientific-domain-bar${item.active?' active':''}" data-flow-domain="${flowEsc(item.domain)}" title="${flowEsc(item.label)}: ${item.documents} ${t('documentos','documents')}; ${item.ready} ${t('prontos para inspeção','ready for inspection')}"><span class="scientific-domain-label">${flowEsc(item.label)}</span><span class="scientific-domain-track"><i style="--flow-volume:${volume}%"><b style="--flow-ready:${ready}%"></b></i></span><span class="scientific-domain-value">${item.documents}</span></button>`
  }).join('')}</div><div class="scientific-domain-legend"><span><i class="volume"></i> ${t('documentos mapeados','mapped documents')}</span><span><i class="ready"></i> ${t('pacote de resultados materializado','materialized result bundle')}</span></div>`
...
function updateIntelligenceContext(){
  ...
  const label=select?.selectedOptions?.[0]?.textContent?.trim()||t('Nenhum domínio selecionado','No domain selected')
  const context=$flow('#scientificFlowContext')
  if(context){
    context.innerHTML=`<div><strong>${t('Domínio em inspeção','Domain under inspection')}</strong><span>${flowEsc(label)}</span></div><div class="scientific-flow-actions"><a class="scientific-flow-action secondary" href="${domain?`/evidence-map.html?domain=${encodeURIComponent(domain)}`:'/evidence-map.html'}">${t('Ver no mapa','View on map')}</a><a class="scientific-flow-action" href="/review.html">${t('Abrir Revisão Humana →','Open Human Review →')}</a></div>`
  }
  refreshFlowLinks()
}
...
function reviewProgressValues(){
  ...
  if(match)return {completed:Number(match[1]),total:Number(match[2]),locked:/travada|enviada|locked|submitted/i.test(meta)}
  ...
  return {completed,total:cards.length,locked:/travada|enviada|locked|submitted/i.test(meta)}
}
...
function renderReviewProgress(){
  ...
  node.innerHTML=`<div class="review-progress-copy"><span>${locked?t('Avaliação travada','Locked assessment'):t('Progresso desta avaliação','Assessment progress')}</span><strong>${completed}/${total||0} ${t('itens','items')}${total?` · ${percent}%`:''}</strong><small>${locked?t('Decisões enviadas e imutáveis para este revisor.','Submitted decisions are immutable for this reviewer.'):t('Progresso operacional da atribuição; não é taxa de inclusão nem resultado científico.','Operational assignment progress; not an inclusion rate or scientific outcome.')}</small></div><div class="review-progress-track" role="progressbar" aria-label="${t('Itens de revisão concluídos','Completed review items')}" aria-valuemin="0" aria-valuemax="${total||0}" aria-valuenow="${completed}"><i style="--review-progress:${percent}%"></i></div>`
}

function updateReviewContext(){
  if(flowStage!=='review')return
  const context=$flow('#scientificFlowContext')
  if(context)context.innerHTML=`<div><strong>${t('Etapa humana isolada','Isolated human stage')}</strong><span>${t('Espaço de trabalho, projeto e aplicação de pesquisa governam esta página. O recorte visual anterior não é importado como decisão.','Workspace, project and research application govern this page. The previous visual slice is not imported as a decision.')}</span></div><a class="scientific-flow-action secondary" href="/intelligence.html">${t('Voltar à Análise','Back to Analysis')}</a>`
}
