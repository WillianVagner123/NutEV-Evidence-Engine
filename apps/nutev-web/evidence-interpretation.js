...
if(page){
  ...
  function mount(){
    ...
    section.innerHTML=`<div class="ei-head"><div><span class="ei-eyebrow">INTERPRETAÇÃO CIENTÍFICA · SOMENTE NAVEGAÇÃO</span><h2>Estruturar → Inspecionar → Revisão humana</h2><p>O trilho organiza a leitura entre superfícies do sistema. Ele não promove elegibilidade, inclusão, certeza, consenso, EvidenceClaim, recomendação ou PRISMA.</p></div></div>${rail()}<div id="evidenceInterpretationBody" class="ei-body" aria-live="polite"></div>`;
  ...
  function renderMap(){
    ...
    body.innerHTML=`<div class="ei-section-head"><div><strong>Navegador de concentração por domínio</strong><span>Contagens estruturais do mapa atual. Clique para aplicar ou remover o filtro de domínio existente.</span></div><span class="ei-boundary-chip">volume ≠ força</span></div><div class="ei-bars">${rows.map(item=>{
      const active=item.domain===selected;
      const width=clamp(100*item.documents/max);
      return `<button type="button" class="ei-bar-button${active?' active':''}" data-ei-map-domain="${esc(item.domain)}" aria-pressed="${active?'true':'false'}"><span class="ei-bar-label"><strong>${esc(item.label)}</strong><small>${item.documents} mapeamentos</small></span><span class="ei-track" aria-hidden="true"><i style="--ei-width:${width}%"></i></span></button>`;
  ...
  function renderIntelligence(){
    ...
    if(!rows.length){body.innerHTML='<div class="ei-empty">Aguardando a síntese estrutural sem uso de ranking científico.</div>';return}
    const max=Math.max(1,...rows.map(item=>item.documents));
    body.innerHTML=`<div class="ei-section-head"><div><strong>Panorama de inspeção por domínio</strong><span>Documentos mapeados e disponibilidade de pacotes de resultados para inspeção.</span></div><span class="ei-boundary-chip">pronto para inspeção ≠ alegação aceita</span></div><div class="ei-domain-grid">${rows.map(item=>{
