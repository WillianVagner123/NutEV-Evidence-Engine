# NutEV System Context — shared automation entrypoint

> Compatibility note: the filename `AI_CONTEXT.md` is historical and remains stable for tooling. It is **not** the product name and does not define NutEV as an AI/LLM interface.

This file is the shared starting point for automated repository tooling and coding/research automation working on NutEV. It points to current sources of truth; it is not itself scientific evidence and it does not create scientific state.

## Product identity

NutEV is a **Scientific Evidence System** (`Sistema de Evidências Científicas`). User-facing surfaces must be named by their scientific or operational function, not by the underlying automation technology. The canonical terminology contract is `docs/PRODUCT_LANGUAGE_SYSTEM.md`.
...
The Article 1 Evidence Context bundle, when materialized, lives on private persistent runtime storage at the historical compatibility path:
...
The `agent_context` directory name is an internal compatibility contract. In the product this layer is called **Evidence Context / Contexto de Evidências**.
...
Automation must retrieve the current canonical A1 master/runtime evidence before making a mutable status claim and must expose disagreement or missing evidence instead of guessing.
...
- treat Evidence Context bundles as navigation/context, not evidence adjudication;
- never convert ranking, route membership or automated output into inclusion/exclusion, quality, certainty, recommendation or PRISMA state;
- never fabricate source results, counts, identifiers, DOI/PMID/URLs, human review or scientific approval;
