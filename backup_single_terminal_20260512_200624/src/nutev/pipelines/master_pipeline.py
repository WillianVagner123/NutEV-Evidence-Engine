...
def _provider_map():
    return {
        ...
        "openalex": lambda q: search_openalex(q, per_page=18),
...
def run_pipeline(settings: NutevSettings, workstreams: list[str], logger) -> dict[str, int]:
    ...
    for ws, queries in qpack.items():
        ...
        query_budget = min(len(queries), QUERY_BUDGET.get(ws, 24))
        ...
        rows_for_download = rows_for_download[:DOWNLOAD_BUDGET.get(ws, 260)]
